        ...
        if !atty::is(atty::Stream::Stdin) {
            ...
            Ok(AsyncPipeIn {
                handle: Some(std::thread::spawn(move || {
                    ...
                    loop {
                        if stdin.read_line(&mut buf).is_ok() && !buf.is_empty() && tx.send(buf.clone()).is_err() {
                            break;
                        }
                        match ry.try_recv() {
...
impl AsyncFileIn {
    pub fn start(file: &str) -> Result<Self, Error> {
        match std::fs::File::open(file).map(std::io::BufReader::new).map_err(|_| Error::FileOpenFailed(file.to_owned())) {
            Ok(mut f) => {
                let (tx, rx) = std::sync::mpsc::channel();
                let (ty, ry) = std::sync::mpsc::channel();
                    Ok(AsyncFileIn {
                    handle: Some(std::thread::spawn(move || {
                        let mut buf = String::with_capacity(1024);
                        loop {
                            if f.read_line(&mut buf).is_ok() && !buf.is_empty() && tx.send(buf.clone()).is_err() {
                                break;
                            }
                            match ry.try_recv() {
                                Err(std::sync::mpsc::TryRecvError::Empty) => {},
